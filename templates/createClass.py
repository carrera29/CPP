...
def generar_clase(nombre_archivo, secciones):
	...
	# Código de las funciones canónicas
	funciones_canonicas = f'''
	public:
	
		{nombre_archivo}();
		~{nombre_archivo}();
		{nombre_archivo}(const {nombre_archivo}& other);
		{nombre_archivo}& operator=(const {nombre_archivo}& other);
...
'''
	...
	for seccion, variables in secciones.items():
		if seccion.lower() == 'public':
			codigo_clase += funciones_canonicas
		else:
...
if __name__ == "__main__":
	...
	for seccion in secciones:
		...
		while True:
			...
			clase = input(f"Especifique 'v' si se trata de una variable o 'f' si es una función, para la sección {seccion} (o 'fin' para terminar): ")
			...
			if clase == 'v':
				...
				nombre_funcion = input(f"Ingrese el nombre de la función para la sección {seccion} (o 'fin' para terminar): ")
				if nombre_funcion.lower() == 'fin':
					break
				secciones[seccion].append(f'{nombre_funcion}')
			elif clase.lower() == 'fin':
				break
